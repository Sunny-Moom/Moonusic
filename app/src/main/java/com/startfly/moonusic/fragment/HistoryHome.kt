...
import com.startfly.moonusic.tools.HistoryMusic

class HistoryHome : Fragment() {
    ...
    override fun onCreateView(
        ...
        val rootView = inflater.inflate(R.layout.home_history, container, false)
        GlobalScope.launch(Dispatchers.Main) {
            progressBar = rootView.findViewById(R.id.musicHisBar)
            more = rootView.findViewById(R.id.loadMoreButton)
            backtop = rootView.findViewById(R.id.resetButton)
            recyclerView = rootView.findViewById(R.id.musicHisView)
            recyclerView.layoutManager = LinearLayoutManager(activity)
            val songList = HistoryMusic().GetHisMusic(1)
    ...
    private suspend fun getDataList(page: Int, pageSize: Int): List<MusicAll> {
        ...
        return HistoryMusic().GetHisMusic(page)
        // 并返回数据列表
    }
